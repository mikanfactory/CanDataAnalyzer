...
function getStateFromStores() {
  return {
    ...
    taskIndex: LayerStore.getTaskIndex(),
  }
}
...
export default class TaskIndexLayer extends React.Component {
  constructor(props) {
    super(props)
  ...
  drawOverlays() {
    const { bounds, taskIndex } = this.state
    ...
    const overlays = rs.map( (position, index) => {
      if (Number(taskIndex[index]) > 0) {
        return new TaskIndexOverlay(taskIndex[index], position, gMap)
      }
    })
  ...
  componentDidUpdate(prevProps, prevState) {
    if(!isEqual(this.state.taskIndex, prevState.taskIndex)) {
